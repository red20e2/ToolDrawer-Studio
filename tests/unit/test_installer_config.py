...
def test_inno_installer_is_per_user_x64_and_versioned_from_build():
    ...
    assert "architecturesallowed=x64compatible" in normalized or "architecturesallowed=x64" in normalized
    assert "architecturesinstallin64bitmode=x64compatible" in normalized or "architecturesinstallin64bitmode=x64" in normalized
    assert "{localappdata}" in text.lower()
    assert "{#AppVersion}" in text
    assert "ToolDrawer Studio.exe" in text


def test_inno_installer_has_start_menu_optional_desktop_and_uninstall():
    ...
    assert "uninstallabledir" not in lower
